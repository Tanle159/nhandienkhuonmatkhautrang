...
def test_on_custom_image(path):
    ...
    if file_extension not in POSSIBLE_EXT:
        raise Exception("phần mở rộng tệp có thể là .png, .jpg, .jpeg")
    if not os.path.exists(path):
        raise FileNotFoundError("tập tin không được tìm thấy")
...
if __name__ == '__main__':
    path = input("vui lòng nhập đường dẫn tệp hình ảnh của bạn:")
